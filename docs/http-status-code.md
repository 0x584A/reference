...
### RESTful API
 ...
 :- | -
---- | ----
`200` | Return success, GET, DELETE request success
`204` | No content, POST request succeeded
`301` | Permanent redirect
`302/307` | Temporary redirect
`304` | Unmodified, since last request
`331` | Username is correct, password is required
`332` | Login required
`400` | Bad request, missing required attribute for API request
`401` | Unauthorized, invalid credentials for authentication will
`403` | Forbidden, the request is not allowed
`404` | not found, unable to access resources
`405` | Method not allowed, the request is not supported
`409` | conflict, conflicting resource already exists
`412` | The request was denied
`422` | Unable to process, the entity cannot be processed
`429` | Too many requests, user exceeded application rate limit
`500` | Server Error, There was a problem with the server while processing the request
`530` | not logged in
